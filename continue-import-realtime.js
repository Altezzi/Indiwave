...
class RealtimeMangaDexAPI {
  ...
  getCoverUrl(cover, mangaId) {
    if (!cover || !cover.attributes?.fileName || !mangaId) return null;
    const baseUrl = 'https://uploads.mangadex.org/covers';
    const fileName = cover.attributes.fileName;
    // Use mangaId instead of cover.id for the correct URL format
    return `${baseUrl}/${mangaId}/${fileName}`;
  }
}

const mangaAPI = new RealtimeMangaDexAPI();
...
async function importOneManga(mangaId, creatorId) {
  try {
    ...
    if (coversData.data && coversData.data.length > 0) {
      coverImageUrl = mangaAPI.getCoverUrl(coversData.data[0], mangaId);
    }
    ...
    try {
      ...
      console.log('⚠️ Could not create publisher sources, using defaults');
    ...
    console.log(`🌐 Added to localhost database in real-time!`);
...
async function continueRealtimeImport() {
  try {
    console.log('🚀 Continuing REALTIME import from MangaDex...');
    console.log('🌐 Adding manga to localhost database in real-time!');
    console.log('📝 Starting from offset 100 to find new manga');
    
    const CREATOR_ID = 'your-admin-1';
    const MAX_MANGA = 200; // Import 200 more manga
    const START_OFFSET = 100; // Start from offset 100 to find new manga
    ...
    console.log(`📊 Will import up to ${MAX_MANGA} more manga starting from offset ${START_OFFSET}`);
      ...
      try {
        ...
        if (result.success) {
          if (result.skipped) {
            ...
            console.log(`🌐 Available on localhost:3000 right now!`);
          }
        } else {
          errorCount++;
        ...
        console.log(`🌐 Check localhost:3000/admin/manga-curation to see new manga!`);
    ...
    console.log('\n🌐 All manga are now available on localhost:3000!');
    console.log('🎨 Next steps:');
    console.log('   1. Visit localhost:3000/admin/manga-curation to review manga');
    console.log('   2. Publish manga you want in the public library');
    console.log('   3. Run this script again to import even more manga');

  } catch (error) {
    console.error('💥 Fatal error during realtime import:', error);
  } finally {
    await prisma.$disconnect();
  }
}

// Start the realtime import
continueRealtimeImport();
