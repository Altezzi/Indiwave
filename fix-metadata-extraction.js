    ...
    for (let i = 0; i < retries; i++) {
      try {
        const response = await fetch(url, {
          headers: {
            'User-Agent': 'indiwave-fixed-metadata-updater/1.0.0',
...
`;
  ...
  async run() {
    console.log('🚀 Starting FIXED METADATA UPDATE for all series...');
    ...
    console.log('\n🎊 FIXED METADATA UPDATE completed!');
...
// Main execution
async function main() {
  const updater = new FixedMetadataUpdater();
  await updater.run();
}

main().catch((error) => {
  console.error('💥 Fixed metadata update failed:', error);
  process.exit(1);
});
