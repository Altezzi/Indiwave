...
`;
  ...
  async importManga(title) {
    try {
      ...
      const cleanTitle = this.getBestTitle(manga).replace(/[<>:"/\\|?*]/g, '_').trim();
      const seriesFolder = path.join(SERIES_DIR, cleanTitle);
      ...
      // Get chapters
      console.log(`📄 Fetching chapters for ${this.getBestTitle(mangaData)}...`);
  ...
  async run() {
    console.log('🚀 Starting SERIES FOLDER creation...');
    ...
    console.log('\n🎊 SERIES FOLDER creation completed!');
    ...
    console.log('\n🎨 Next steps for server transfer:');
    console.log('   1. Copy the entire "series" folder to your flash drive');
    console.log('   2. Transfer to your server PC');
    console.log('   3. Place the "series" folder in your project root');
    console.log('   4. Each series folder contains everything needed:');
    ...
    console.log('   5. Use the metadata to populate your database on the server');
  }
}

// Main execution
async function main() {
  const creator = new SeriesFolderCreator();
  await creator.run();
