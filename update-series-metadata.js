...
class MetadataUpdater {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        const response = await fetch(url, {
          headers: {
            'User-Agent': 'indiwave-metadata-updater/1.0.0',
  ...
  // Better authors and artists extraction
...
`;
  ...
  async run() {
    console.log('🚀 Starting METADATA UPDATE for all series...');
